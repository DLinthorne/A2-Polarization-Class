...
Bool_t	PPhysics::FillTheta(const GTreeParticle& tree, Int_t particle_index, TH1* Tprompt, TH1* Trandom)
{
	for (Int_t i = 0; i < GetTagger() -> GetNTagged(); i++)
	{	
		if (GetTrigger() -> GetNErrors() != 0) continue;
				
		//FillEnergy(particle_index, i, Eprompt, Erandom);		
	
	        //FIX ME: not to sure if This is beam energy
		if(GetTagger() -> GetTaggedEnergy(i) < 275) continue;
		if(GetTagger() -> GetTaggedEnergy(i) > 300) continue;
...
Bool_t PPhysics::FillThetaPair(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index,TH1* Tprompt, TH1* Trandom)
{
	...
	if (Prompt) Tprompt -> Fill(GetTracks() -> GetTheta(particle_index));
	if (Random) Trandom -> Fill(GetTracks() -> GetTheta(particle_index));
